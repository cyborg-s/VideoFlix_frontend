...
export class Signup implements OnInit {
  ...
  passwordError: string = '';  // NEU: Fehlermeldung zwischen den Feldern
...
signUp(): void {
  this.passwordError = ''; // reset Fehleranzeige vor Prüfung
  ...
  this.authService
    ...
    .subscribe({
      next: (res) => {
        this.passwordError = '';
        this.showSuccessToastWithButton(res.message || 'Registrierung erfolgreich. Bitte E-Mail bestätigen.');
      },
      error: (err) => {
        const serverError =
        ...
        if (serverError) {
          ...
          this.showToast('Bitte überprüfe deine Eingaben und versuche es erneut.');
        }
      },
    });
}
